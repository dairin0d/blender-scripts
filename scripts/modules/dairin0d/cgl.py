...
class CGL:
    ...
    def polygon_stipple_from_list(self, L, zeros=False, tile=True):
        if isinstance(L, str):
            L = L.strip("\n").split("\n")
...
def fill_CGL():
    ...
    def State(name, doc, *params):
        ...
        args_info = []
        for arg_type, arg_key in params:
            getter_specific = "{0}"
            setter_specific = "{0}"
            ...
            if isinstance(arg_type, set):
                ...
                data_size = 1
                state_getter = bgl.glGetIntegerv
            ...
            if data_size == 1:
                ...
                setter_specific = setter_specific.format(
                    "Buf(_type%s, %s, {0})" % (arg_id, data_size))
            
            state_id = getattr(bgl, arg_key)
        ...
        for i in range(len(args_info)):
            ...
            getter_lines.append(
                "   _getter{0}(_state_id{0}, _buf{0})".format(i))
...
""".format(get_body, set_body)
    ...
    State('glColor:4fv', "", ("float:4", 'GL_CURRENT_COLOR'))
    State('glColor3:fv', "", ("float:3", 'GL_CURRENT_COLOR'))
    State('glBlendFunc', "", # Somewhy CONST modes aren't present in bgl %)
        ...
        'SRC_ALPHA_SATURATE'}, 'GL_BLEND_SRC'),
        ({'ZERO', 'ONE', 'SRC_COLOR', 'ONE_MINUS_SRC_COLOR', 'DST_COLOR',
        'ONE_MINUS_DST_COLOR', 'SRC_ALPHA', 'ONE_MINUS_SRC_ALPHA',
        'DST_ALPHA', 'ONE_MINUS_DST_ALPHA'}, 'GL_BLEND_DST'))
